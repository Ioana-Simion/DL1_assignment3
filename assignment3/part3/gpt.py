...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply attention to the values: (B, nh, T, hs)
...
class GPT(nn.Module):
    ...
    def generate(self, idx: torch.LongTensor, max_new_tokens: int, temperature:float = 1.0, do_sample:bool = False, top_k:int = None):
        ...
        for _ in range(max_new_tokens):
            ...
            # Optionally consider top-k logits for sampling
            ...
            if do_sample:
                next_token = torch.multinomial(probabilities, 1)  # This will be of shape [batch_size, 1]
            else:
                next_token = torch.argmax(probabilities, dim=-1, keepdim=True)  # This will also be of shape [batch_size, 1]
